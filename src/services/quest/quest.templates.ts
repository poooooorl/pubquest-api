...
export const QUEST_TEMPLATES: Record<string, QuestDefinition> = {
  // ==========================================================================
  // Tutorial Quests
  // ==========================================================================
  ...
  FIRST_STEPS: {
    ...
    title: "First Steps",
    description:
      "Welcome to PubQuest! Visit your first pub to begin your journey.",
  ...
  THE_LEGENDARY_PINT: {
    ...
    description:
      "Kat the Serveress has heard tales of a legendary drink. Help her track it down.",
  ...
  TASTE_TESTER: {
    ...
    description:
      "Sample drinks from 5 different venues to become a true connoisseur.",
 ...
 */
export function getQuestTemplate(questId: number): QuestDefinition | undefined {
  return Object.values(QUEST_TEMPLATES).find((q) => q.id === questId);
}
...
export function getQuestsForLevel(level: number): QuestDefinition[] {
  return Object.values(QUEST_TEMPLATES).filter((q) => {
    if (!q.prerequisites?.minLevel) return true;
