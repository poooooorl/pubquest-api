...
export const getParty = async (
  ...
  try {
    ...
    const membersRes = await pool.query(
      ...
      WHERE pm.party_id = $1
      ORDER BY is_leader DESC, pm.joined_at ASC`,
      [partyId],
    );
...
export const createParty = async (
  ...
  try {
    ...
    const partyDetailsRes = await client.query(
      `SELECT 
        p.id,
        p.name,
        p.invite_code,
...
export const joinParty = async (
  ...
  try {
    ...
    const partyRes = await pool.query(
      `SELECT id, name FROM parties WHERE invite_code = $1`,
      [inviteCode],
    );
...
export const deleteParty = async (
  ...
  try {
    ...
    const partyRes = await client.query(
      `SELECT id, leader_id, name FROM parties WHERE id = $1`,
      [partyId],
    );
...
export const inviteUser = async (
  ...
  try {
    ...
    const inviteeRes = await pool.query(`SELECT id FROM users WHERE id = $1`, [
      inviteeId,
    ]);
...
export const acceptInvite = async (
  ...
  try {
    ...
    const inviteRes = await client.query(
      `SELECT * FROM party_invites WHERE id = $1 AND invitee_id = $2 AND status = 'PENDING'`,
      [inviteId, userId],
    );
    ...
    const inParty = await client.query(
      `SELECT party_id FROM party_members WHERE user_id = $1`,
      [userId],
    );
    ...
    await client.query(
      `UPDATE party_invites SET status = 'ACCEPTED', updated_at = NOW() WHERE id = $1`,
      [inviteId],
    );
...
export const rejectInvite = async (
  ...
  try {
    ...
    await pool.query(
      `UPDATE party_invites SET status = 'REJECTED', updated_at = NOW() WHERE id = $1`,
      [inviteId],
    );
...
export const getMyInvites = async (
  ...
  try {
    ...
    const receivedInvites = await pool.query(
      `SELECT 
...
export const requestToJoin = async (
  ...
  try {
    ...
    const requestRes = await pool.query(
      `INSERT INTO party_join_requests (party_id, user_id, status)
       VALUES ($1, $2, 'PENDING')
...
export const acceptJoinRequest = async (
  ...
  try {
    ...
    const requestRes = await client.query(
      `SELECT * FROM party_join_requests WHERE id = $1 AND status = 'PENDING'`,
      [requestId],
    );
    ...
    await client.query(
      `INSERT INTO party_members (party_id, user_id) VALUES ($1, $2)`,
      [request.party_id, request.user_id],
    );

    // 5. Update request status
    await client.query(
      `UPDATE party_join_requests SET status = 'ACCEPTED', updated_at = NOW() WHERE id = $1`,
      [requestId],
    );
...
export const rejectJoinRequest = async (
  ...
  try {
    ...
    await pool.query(
      `UPDATE party_join_requests SET status = 'REJECTED', updated_at = NOW() WHERE id = $1`,
      [requestId],
    );
...
export const getPartyRequests = async (
  req: AuthRequest,
  res: Response,
): Promise<void> => {
  try {
    ...
    if (
      partyRes.rows.length === 0 ||
      partyRes.rows[0].leader_id !== parseInt(userId as string)
    ) {
      res
        .status(403)
