 ...
 */
export const getDialogueNodes = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
 ...
 */
export const getDialogueNode = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    try {
      const result = await client.query(
        `SELECT * FROM dialogue_nodes WHERE id = $1`,
        [nodeId],
      );
 ...
 */
export const createDialogueNode = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    try {
      ...
      if (parent_node_id) {
        const parentCheck = await client.query(
          `SELECT id FROM dialogue_nodes WHERE id = $1 AND npc_id = $2`,
          [parent_node_id, npcId],
        );
 ...
 */
export const updateDialogueNode = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    try {
      ...
      if (
        ...
        if (parent_node_id !== null) {
          const parentCheck = await client.query(
            `SELECT id FROM dialogue_nodes WHERE id = $1 AND npc_id = $2`,
            [parent_node_id, existingNode.npc_id],
          );
 ...
 */
export const deleteDialogueNode = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    try {
      ...
      const childrenCheck = await client.query(
        `SELECT COUNT(*) FROM dialogue_nodes WHERE parent_node_id = $1`,
        [nodeId],
      );
      ...
      // Delete the node
      const result = await client.query(
        `DELETE FROM dialogue_nodes WHERE id = $1 RETURNING *`,
        [nodeId],
      );
