...
exports.defaultParties = void 0;
exports.seedParties = seedParties;
exports.seedPartyInvites = seedPartyInvites;
exports.defaultParties = [
    {
        name: "Bum Squad",
        leaderUsername: "paul",
...
async function seedParties(client, users) {
    const createdParties = [];
    for (const partyConfig of exports.defaultParties) {
        const leader = users.find((u) => u.username === partyConfig.leaderUsername);
        if (!leader)
            continue;
        const inviteCode = Math.random().toString(36).substring(2, 8).toUpperCase();
        const partyRes = await client.query(`INSERT INTO parties (name, leader_id, invite_code)
       VALUES ($1, $2, $3)
       RETURNING id, name, leader_id, invite_code`, [partyConfig.name, leader.id, inviteCode]);
        // Add leader as member
        await client.query(`INSERT INTO party_members (party_id, user_id)
       VALUES ($1, $2)`, [partyRes.rows[0].id, leader.id]);
        createdParties.push({ ...partyRes.rows[0], leader });
        // Add other members
        for (const username of partyConfig.memberUsernames) {
            const user = users.find((u) => u.username === username);
            if (user) {
                await client.query(`INSERT INTO party_members (party_id, user_id) VALUES ($1, $2)`, [partyRes.rows[0].id, user.id]);
            }
        }
    }
...
// Seed party invites
async function seedPartyInvites(client, users, parties) {
    let inviteCount = 0;
    ...
    if (paul && bumSquad) {
        ...
        for (const username of inviteesToPaul) {
            const user = users.find((u) => u.username === username);
            if (user) {
                await client.query(`INSERT INTO party_invites (party_id, inviter_id, invitee_id, status)
           VALUES ($1, $2, $3, 'PENDING')`, [bumSquad.id, paul.id, user.id]);
                inviteCount++;
            }
        }
    }
    ...
    if (paul && londonCrawlers) {
        ...
        if (diana) {
            await client.query(`INSERT INTO party_invites (party_id, inviter_id, invitee_id, status)
         VALUES ($1, $2, $3, 'PENDING')`, [londonCrawlers.id, diana.id, paul.id]);
            inviteCount++;
        }
    }
    if (paul && pubMasters) {
        ...
        if (henry) {
            await client.query(`INSERT INTO party_invites (party_id, inviter_id, invitee_id, status)
         VALUES ($1, $2, $3, 'PENDING')`, [pubMasters.id, henry.id, paul.id]);
            inviteCount++;
        }
    }
