...
exports.seedNPCs = seedNPCs;
exports.linkNPCsToQuests = linkNPCsToQuests;
async function seedNPCs(client, venues) {
    ...
    const npcs = [
        {
            ...
            description: "A friendly serveress at Bubala King's Cross who knows all the regulars and their favorite drinks. She's always ready with a smile and a quick chat.",
            greeting_text: "Welcome to Bubala King's Cross! Can I get you something to drink or perhaps a quest to embark on?",
            dialogue_tree: {
                greeting: {
        ...
        },
        {
            ...
            avatar_url: undefined,
            description: "A weathered old bartender who has seen countless adventurers pass through. He always has a quest or two for those willing to help.",
            venue_id: 1, // The Rusty Spoon
            greeting_text: "Ah, a fresh face! Care to help an old man with some tasks?",
            dialogue_tree: {
                greeting: {
        ...
        },
        {
            ...
            avatar_url: undefined,
            description: "A cloaked stranger who appears at various locations. Their true identity is unknown, but their quests are always intriguing.",
        ...
        },
        {
            name: "Lady Penelope",
            avatar_url: undefined,
            description: "A noble lady who frequents the finest establishments. She seeks adventurers for more sophisticated quests.",
            venue_id: 2, // The Golden Lion
            greeting_text: "Oh my, you look like someone who can handle delicate matters!",
            dialogue_tree: {
                greeting: {
...
async function linkNPCsToQuests(client, npcs, quests) {
    ...
    for (const link of links) {
        ...
        if (npc && quest) {
            ...
            await client.query(`INSERT INTO npc_quests (npc_id, quest_id, is_repeatable, level_requirement)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (npc_id, quest_id) DO NOTHING`, [npc.id, quest.id, link.is_repeatable, link.level_requirement]);
            // Also update the quest's giver_npc_id field if not already set
            await client.query(`UPDATE quests 
         SET giver_npc_id = $1, turn_in_npc_id = $1
         WHERE id = $2 AND giver_npc_id IS NULL`, [npc.id, quest.id]);
            console.log(`   ✅ Linked ${npc.name} -> ${quest.title}`);
        }
        else {
            console.log(`   ⚠️  Could not link: ${link.npc_name} -> ${link.quest_title}`);
            if (!npc)
                console.log(`      NPC not found: ${link.npc_name}`);
            if (!quest)
                console.log(`      Quest not found: ${link.quest_title}`);
        }
    }
}
