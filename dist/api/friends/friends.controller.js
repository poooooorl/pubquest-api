...
exports.removeFriend = exports.getPendingRequests = exports.getFriends = exports.rejectFriendRequest = exports.acceptFriendRequest = exports.sendFriendRequest = void 0;
...
const sendFriendRequest = async (req, res) => {
    ...
    try {
        ...
        const addresseeCheck = await pool_1.default.query(`SELECT id FROM users WHERE id = $1`, [addresseeId]);
        if (addresseeCheck.rows.length === 0) {
            res.status(404).json({ error: "User not found" });
            return;
        }
        // Check if friendship already exists (in either direction)
        const existingFriendship = await pool_1.default.query(`SELECT * FROM friendships 
       WHERE (requester_id = $1 AND addressee_id = $2) 
          OR (requester_id = $2 AND addressee_id = $1)`, [userId, addresseeId]);
        ...
        const result = await pool_1.default.query(`INSERT INTO friendships (requester_id, addressee_id, status)
       VALUES ($1, $2, 'PENDING')
       RETURNING *`, [userId, addresseeId]);
        // Invalidate cache for both users (in case they were displaying pending requests)
        await Promise.all([
            (0, cache_service_1.deleteCached)(cache_service_1.CacheKeys.USER_FRIENDS(userId)),
            (0, cache_service_1.deleteCached)(cache_service_1.CacheKeys.USER_FRIENDS(addresseeId)),
        ]);
...
const acceptFriendRequest = async (req, res) => {
    ...
    try {
        ...
        const friendshipCheck = await pool_1.default.query(`SELECT * FROM friendships WHERE id = $1 AND addressee_id = $2 AND status = 'PENDING'`, [friendshipId, userId]);
        if (friendshipCheck.rows.length === 0) {
            res.status(404).json({ error: "Friend request not found" });
            return;
        }
        // Update status to accepted
        const result = await pool_1.default.query(`UPDATE friendships 
       SET status = 'ACCEPTED', updated_at = NOW()
       WHERE id = $1
       RETURNING *`, [friendshipId]);
        // Invalidate cache for both users
        const acceptedFriendship = result.rows[0];
        await Promise.all([
            (0, cache_service_1.deleteCached)(cache_service_1.CacheKeys.USER_FRIENDS(acceptedFriendship.requester_id)),
            (0, cache_service_1.deleteCached)(cache_service_1.CacheKeys.USER_FRIENDS(acceptedFriendship.addressee_id)),
        ]);
        res.json({ friendship: result.rows[0] });
    }
    catch (error) {
...
const rejectFriendRequest = async (req, res) => {
    ...
    try {
        // Check that the friendship exists and user is the addressee
        const friendship = await pool_1.default.query(`SELECT * FROM friendships WHERE id = $1 AND addressee_id = $2 AND status = 'PENDING'`, [friendshipId, userId]);
        if (friendship.rows.length === 0) {
            res.status(404).json({ error: "Friend request not found" });
            return;
        }
        // Delete the friendship (or update to REJECTED if you want to keep history)
        await pool_1.default.query(`DELETE FROM friendships WHERE id = $1`, [friendshipId]);
...
const getFriends = async (req, res) => {
    ...
    try {
        ...
        if (!search && page === 1) {
            const cached = await (0, cache_service_1.getCached)(cache_service_1.CacheKeys.USER_FRIENDS(userId));
            if (cached) {
                res.json(cached);
                return;
            }
        ...
        try {
            ...
            if (search) {
                dataWhereClauses.push(`(CASE WHEN f.requester_id = $3 THEN u2.username ELSE u1.username END ILIKE $${dataParamIndex} OR 
           CASE WHEN f.requester_id = $3 THEN u2.email ELSE u1.email END ILIKE $${dataParamIndex})`);
                dataQueryParams.push(`%${search}%`);
                dataParamIndex++;
                countWhereClauses.push(`(CASE WHEN f.requester_id = $1 THEN u2.username ELSE u1.username END ILIKE $${countParamIndex} OR 
      ...
      `;
            ...
            if (!search && page === 1) {
                await (0, cache_service_1.setCached)(cache_service_1.CacheKeys.USER_FRIENDS(userId), response, 300);
            }
...
const removeFriend = async (req, res) => {
    ...
    try {
        ...
        const friendship = await pool_1.default.query(`SELECT * FROM friendships 
       WHERE id = $1 AND (requester_id = $2 OR addressee_id = $2) AND status = 'ACCEPTED'`, [friendshipId, userId]);
        if (friendship.rows.length === 0) {
            res.status(404).json({ error: "Friendship not found" });
            return;
        }
        const friendshipRow = friendship.rows[0];
        const otherUserId = friendshipRow.requester_id === userId
        ...
        await Promise.all([
            (0, cache_service_1.deleteCached)(cache_service_1.CacheKeys.USER_FRIENDS(userId)),
            (0, cache_service_1.deleteCached)(cache_service_1.CacheKeys.USER_FRIENDS(otherUserId)),
        ]);
