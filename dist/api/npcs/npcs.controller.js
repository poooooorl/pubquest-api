...
exports.getContextualDialogue = exports.getNPCDialogue = exports.acceptQuestFromNPC = exports.getNPCQuests = exports.getNPCById = exports.getNPCs = void 0;
...
const getNPCs = async (req, res) => {
    ...
    `;
        ...
        // Filter by proximity
        if (lat && lng && radius) {
            conditions.push(`ST_DWithin(n.location, ST_SetSRID(ST_MakePoint($${paramIndex + 1}, $${paramIndex}), 4326)::geography, $${paramIndex + 2})`);
 ...
 */
const getNPCQuests = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        if (!userId) {
    ...
    `;
        const [availableResult, activeResult] = await Promise.all([
            pool_1.default.query(availableQuery, [id, userId]),
            pool_1.default.query(activeQuery, [id, userId]),
        ]);
        res.json({
 ...
 */
const acceptQuestFromNPC = async (req, res) => {
    ...
    try {
        ...
        const npcQuestCheck = await client.query(`SELECT nq.*, u.level as user_level
       FROM npc_quests nq
       CROSS JOIN users u
       WHERE nq.npc_id = $1 
         AND nq.quest_id = $2
         AND u.id = $3`, [id, quest_id, userId]);
        ...
        const existingQuest = await client.query(`SELECT * FROM user_quests 
       WHERE user_id = $1 AND quest_id = $2 AND status = 'ACTIVE'`, [userId, quest_id]);
        ...
        if (!npcQuest.is_repeatable) {
            const completedQuest = await client.query(`SELECT * FROM user_quests 
         WHERE user_id = $1 AND quest_id = $2 AND status = 'COMPLETED'`, [userId, quest_id]);
       ...
       ON CONFLICT (user_id, quest_id) DO NOTHING`, [userId, quest_id]);
        // Initialize progress for all objectives
        const objectives = await client.query(`SELECT id FROM quest_objectives WHERE quest_id = $1`, [quest_id]);
 ...
 */
const getNPCDialogue = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        if (!userId) {
        ...
        const npcResult = await pool_1.default.query(`SELECT id, name, description, avatar_url, greeting_text, dialogue_tree
       FROM npcs WHERE id = $1`, [id]);
        ...
        if (greetingNode?.check_quest && greetingNode?.if_active) {
            ...
            const questCheck = await pool_1.default.query(`SELECT uq.status 
         FROM user_quests uq
         JOIN quests q ON uq.quest_id = q.id
         WHERE uq.user_id = $1 AND q.title = $2`, [userId, greetingNode.check_quest]);
 ...
 */
const getContextualDialogue = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // Get NPC with dialogue tree
        const npcResult = await pool_1.default.query(`SELECT id, name, dialogue_tree FROM npcs WHERE id = $1`, [id]);
        if (npcResult.rows.length === 0) {
            return res.status(404).json({ error: "NPC not found" });
        }
        const npc = npcResult.rows[0];
        ...
        if ((dialogueNode.show_quests || dialogueNode.quest_title) &&
            dialogueNode.if_no_work) {
            // Get available quests
            const availableQuestsResult = await pool_1.default.query(`SELECT q.id
         FROM npc_quests nq
         JOIN quests q ON nq.quest_id = q.id
         LEFT JOIN user_quests uq ON q.id = uq.quest_id AND uq.user_id = $2
         CROSS JOIN users u
         ...
         LIMIT 1`, [id, userId]);
            ...
            const activeQuestsResult = await pool_1.default.query(`SELECT q.title
         FROM npc_quests nq
         JOIN quests q ON nq.quest_id = q.id
         JOIN user_quests uq ON q.id = uq.quest_id AND uq.user_id = $2
...
exports.getContextualDialogue = getContextualDialogue;
