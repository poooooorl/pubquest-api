...
exports.QuestEditorService = void 0;
...
class QuestEditorService {
     ...
     */
    static async saveQuest(questData) {
        ...
        try {
            ...
            if (questId && questId > 0) {
                // UPDATE existing
                await client.query(`UPDATE quests SET 
           ...
           WHERE id = $11`, [
                ...
                // INSERT new
                const res = await client.query(`INSERT INTO quests (
           ...
           RETURNING id`, [
            ...
            if (questData.objectives && questData.objectives.length > 0) {
                for (const [index, obj] of questData.objectives.entries()) {
                    await client.query(`INSERT INTO quest_objectives (
     ...
     */
    static async getQuestForEditor(questId) {
        const client = await pool_1.default.connect();
        try {
            // Get Quest
            const questRes = await client.query(`SELECT * FROM quests WHERE id = $1`, [questId]);
            if (questRes.rows.length === 0)
                return null;
            const quest = questRes.rows[0];
            // Get Objectives
            const objRes = await client.query(`SELECT * FROM quest_objectives WHERE quest_id = $1 ORDER BY order_index ASC`, [questId]);
            return {
                id: quest.id,
                title: quest.title,
...
exports.QuestEditorService = QuestEditorService;
