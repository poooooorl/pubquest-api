...
exports.QuestEngine = void 0;
const pool_1 = __importDefault(require("../../db/pool"));
const quest_handlers_1 = require("../../services/quest/handlers/quest.handlers");
...
class QuestEngine {
    ...
    static async processEvent(event) {
        ...
        try {
            console.log(`⚙️  QuestEngine: Processing ${event.type} event for User ${event.userId}`);
     ...
     */
    static async getAvailableQuests(client, userId) {
    ...
    `;
        ...
        for (const quest of allQuests) {
            const canAccept = await this.checkPrerequisites(client, userId, userLevel, quest.prerequisites);
            if (canAccept) {
     ...
     */
    static async checkPrerequisites(client, userId, userLevel, prerequisites) {
        if (!prerequisites) {
        ...
        if (prerequisites.completedQuests &&
            prerequisites.completedQuests.length > 0) {
            const completedCheck = await client.query(`SELECT COUNT(*) as count FROM user_quests 
         WHERE user_id = $1 
         AND quest_id = ANY($2::int[]) 
         AND status = 'COMPLETED'`, [userId, prerequisites.completedQuests]);
     ...
     */
    static async acceptQuest(client, userId, questId) {
        await client.query("BEGIN");
        try {
            // Verify quest is available (not already active)
            const existingQuest = await client.query(`SELECT status FROM user_quests WHERE user_id = $1 AND quest_id = $2`, [userId, questId]);
         ...
         DO UPDATE SET status = 'ACTIVE', accepted_at = NOW()`, [userId, questId]);
            // Create objective progress entries
            const objectives = await client.query(`SELECT id FROM quest_objectives WHERE quest_id = $1`, [questId]);
            for (const obj of objectives.rows) {
                await client.query(`INSERT INTO user_objective_progress (user_id, objective_id, current_progress, is_completed)
           VALUES ($1, $2, 0, FALSE)
           ON CONFLICT (user_id, objective_id) DO NOTHING`, [userId, obj.id]);
            }
     ...
     */
    static async turnInQuest(client, userId, questId) {
        await client.query("BEGIN");
        try {
            // Verify quest is READY
            const questCheck = await client.query(`SELECT status FROM user_quests WHERE user_id = $1 AND quest_id = $2`, [userId, questId]);
     ...
     */
    static async findMatchingObjectives(client, event) {
        const query = `
      SELECT DISTINCT 
        qo.id, 
        qo.quest_id, 
     ...
     */
    static async evaluateObjectives(client, event, objectives) {
        const completed = [];
        const questsCompleted = [];
        let totalXp = 0;
        let totalGold = 0;
        for (const objective of objectives) {
            const handler = quest_handlers_1.ObjectiveHandlers[objective.type];
            if (!handler) {
                console.warn(`⚠️  No handler found for objective type: ${objective.type}`);
                continue;
            }
            ...
            if (result.newProgress > currentProgress) {
                // Progress increased!
                const wasCompleted = await this.updateObjectiveProgress(client, event.userId, objective, result.newProgress, result.satisfied);
                if (wasCompleted) {
                    completed.push(objective);
                    totalXp += objective.reward_xp || 0;
                    totalGold += objective.reward_gold || 0;
                    // Check if quest is complete
                    const questComplete = await this.checkQuestCompletion(client, event.userId, objective.quest_id);
                    if (questComplete) {
     ...
     */
    static async updateObjectiveProgress(client, userId, objective, newProgress, isComplete) {
        await client.query("BEGIN");
        try {
            // Update progress
            const updateRes = await client.query(`UPDATE user_objective_progress 
         SET current_progress = $3,
             is_completed = $4,
             completed_at = CASE WHEN $4 = TRUE THEN NOW() ELSE completed_at END
         WHERE user_id = $1 
           AND objective_id = $2
           AND (current_progress < $3 OR is_completed = FALSE)`, [userId, objective.id, newProgress, isComplete]);
            ...
            if (isComplete) {
                console.log(`✅ Objective ${objective.id} completed! (${newProgress}/${objective.target_count})`);
                ...
                console.log(`📊 Objective ${objective.id} progress: ${newProgress}/${objective.target_count}`);
            }
            await client.query("COMMIT");
            return isComplete;
        }
     ...
     */
    static async grantObjectiveReward(client, userId, objective) {
        ...
        if (xp > 0 || gold > 0) {
            ...
            const newLevel = (0, level_service_1.calculateLevel)(newXP);
            await client.query(`UPDATE users SET xp = $1, level = $2, gold = gold + $3 WHERE id = $4`, [newXP, newLevel, gold, userId]);
            if (xp > 0)
                console.log(`  +${xp} XP awarded`);
            if (gold > 0)
                console.log(`  +${gold} Gold awarded`);
        }
    }
    /**
     ...
     */
    static async checkQuestCompletion(client, userId, questId) {
        // Check for remaining incomplete objectives
        const remaining = await client.query(`SELECT id FROM user_objective_progress 
       WHERE user_id = $1 
         AND objective_id IN (
           SELECT id FROM quest_objectives WHERE quest_id = $2
         )
         AND is_completed = FALSE`, [userId, questId]);
        // Quest not yet complete
        if (remaining.rows.length > 0) {
            return false;
        }
        // Mark quest as READY (atomic check to prevent duplicates)
        const questUpdateRes = await client.query(`UPDATE user_quests 
       SET status = 'READY', completed_at = NOW()
       WHERE user_id = $1 
         AND quest_id = $2 
         AND status = 'ACTIVE'`, [userId, questId]);
     ...
     */
    static async grantQuestRewards(client, userId, questId) {
        const rewardRes = await client.query(`SELECT reward_xp, reward_gold FROM quests WHERE id = $1`, [questId]);
        ...
        const newLevel = (0, level_service_1.calculateLevel)(newXP);
        await client.query(`UPDATE users SET xp = $1, level = $2, gold = gold + $3 WHERE id = $4`, [newXP, newLevel, gold, userId]);
        console.log(`🏆 Quest ${questId} rewards granted: +${xp} XP, +${gold} Gold`);
        return rewards;
    }
}
exports.QuestEngine = QuestEngine;
